...
import Prelude
       hiding (Monoid, Foldable, mappend, mempty, fold, foldMap)
...
size :: Rose a -> Int
size (_ :> xs) = 1 + foldl (\a t -> a + size t) 0 xs 

leaves :: Rose a -> Int
leaves (_ :> []) = 1
leaves (_ :> xs) = foldl (\a t -> a + leaves t) 0 xs
...
class Functor f => Foldable f where
  ...
  foldMap c = fold . fmap c
  
instance Foldable Rose where
  fold (x :> []) = x
  fold (x :> xs) = foldl (\a r -> a `mappend` (fold r)) x xs
